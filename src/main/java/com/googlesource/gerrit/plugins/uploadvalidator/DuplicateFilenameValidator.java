...
public class DuplicateFilenameValidator implements CommitValidationListener {
    ...
    return new AbstractModule() {
      ...
      protected void configure() {
        ...
        bind(ProjectConfigEntry.class)
            .annotatedWith(Exports.named(KEY_REJECT_DUPLICATE_FILENAMES))
            .toInstance(new ProjectConfigEntry("Reject Duplicate Filenames",
  ...
  public static String KEY_REJECT_DUPLICATE_FILENAMES =
      "rejectDuplicateFilenames";
  ...
  private boolean areDuplicateFilenamesAllowed(CommitReceivedEvent receiveEvent)
    ...
    return cfg.getBoolean(KEY_REJECT_DUPLICATE_FILENAMES, false);
  }
    ...
    if (messages.isEmpty()) {
      try (TreeWalk tw = new TreeWalk(repo)) {
        ...
        if (c.getParentCount() > 0) {
          ...
          while (tw.next()) {
            handleTreeWalkEntry(tw, files, deletedFiles, messages);
          }
        }
      }
    }
  ...
  private static void checkForDuplicatesWithinThisCommit(Set<String> files,
      List<CommitValidationMessage> messages) {
    Map<String, String> paths = new HashMap<>();
    for (String file : files) {
      if (paths.containsKey(file.toLowerCase(Locale.ENGLISH))) {
        messages
            .add(new CommitValidationMessage(file + ": filename collides with "
