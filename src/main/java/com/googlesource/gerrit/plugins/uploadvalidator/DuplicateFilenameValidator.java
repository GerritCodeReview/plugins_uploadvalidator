...
public class DuplicateFilenameValidator implements CommitValidationListener {
  ...
  private boolean areDuplicateFilenamesAllowed(CommitReceivedEvent receiveEvent)
    ...
    return cfg.getBoolean(KEY_ALLOW_DUPLICATE_FILENAMES, true);
  }
  ...
  List<CommitValidationMessage> performValidation(Repository repo, RevCommit c)
    ...
    if (messages.isEmpty()) {
      // Don't do this every time because checking against the whole tree is
      // expensive
  ...
  private void checkForDuplicatesWithinThisCommit(Set<String> files,
      List<CommitValidationMessage> messages) {
