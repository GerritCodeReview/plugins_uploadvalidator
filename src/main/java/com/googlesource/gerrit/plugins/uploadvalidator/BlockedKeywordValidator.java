...
public class BlockedKeywordValidator implements CommitValidationListener {
  ...
  public List<CommitValidationMessage> onCommitReceived(
    ...
    try {
      ...
      throw new CommitValidationException(
          "Upload validator failed to find project: "
              + receiveEvent.project.getName(), e);
    } catch (IOException | ExecutionException e) {
      throw new CommitValidationException(
          "Upload validator errored while validating", e);
    }
  ...
  List<CommitValidationMessage> performValidation(
    ...
    checkCommitMessageForBlockedKeywords(
        blockedKeywordPartterns, messages, c.getFullMessage());
