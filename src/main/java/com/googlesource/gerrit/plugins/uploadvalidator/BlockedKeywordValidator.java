...
public class BlockedKeywordValidator implements CommitValidationListener, CommentValidator {
  ...
  // This is used for a configuration option to append a custom message when blocked keywords are detected.
  private static final String KEY_CHECK_BLOCKED_OPTIONAL_MESSAGE = "rejectionMessage";
    ...
    try {
      ...
      if (isActive(cfg)
        ...
        try (Repository repo = repoManager.openRepository(receiveEvent.project.getNameKey())) {
          ...
          if (!messages.isEmpty()) {
            String rejectionMessage = cfg.getString(KEY_CHECK_BLOCKED_OPTIONAL_MESSAGE);
            rejectionMessage = rejectionMessage == null ? "" : " " + rejectionMessage;
            throw new CommitValidationException(
                "includes files containing blocked keywords." + rejectionMessage, messages);
          }
        }
      }
