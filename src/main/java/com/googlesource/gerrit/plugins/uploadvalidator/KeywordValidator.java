...
public class KeywordValidator implements CommitValidationListener {
      ...
      if (blockedKeywordPatterns.size() > 0) {
        try (Repository repo = repoManager.openRepository(
          ...
          List<CommitValidationMessage> messages = performValidation(
              repo, receiveEvent.commit, blockedKeywordPatterns);
          if (!messages.isEmpty()) {
  ...
  List<CommitValidationMessage> performValidation(Repository repo,
      RevCommit c, List<Pattern> blockedKeywordPatterns) throws IOException {
  ...
  private void checkFileForBlockedKeywords(List<Pattern> blockedKeywordPatterns,
  ...
  private void checkLineForBlockedKeywords(List<Pattern> invalidKeywordPatterns,
