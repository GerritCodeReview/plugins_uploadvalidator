...
public class DuplicatePathnameValidator implements CommitValidationListener {
  ...
  void checkForDuplicatesAgainstTheWholeTree(TreeWalk tw,
      Set<String> changed, List<CommitValidationMessage> messages)
          throws IOException {
    Map<String, String> folders = allPaths(changed);

    while (tw.next()) {
      String currentPath = tw.getPathString();

      if (isDeleted(tw) || changed.contains(currentPath)) {
        continue;
      }

      String potentialDuplicate = folders.get(currentPath.toLowerCase(locale));
