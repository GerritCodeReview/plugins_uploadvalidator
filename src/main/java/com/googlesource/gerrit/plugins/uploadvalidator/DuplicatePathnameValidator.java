...
public class DuplicatePathnameValidator implements CommitValidationListener {
    ...
    return new AbstractModule() {
      private List<String> getAvailableLocales() {
        return ImmutableList.copyOf(
            Collections2.transform(Arrays.asList(Locale.getAvailableLocales()),
  ...
  static List<CommitValidationMessage> performValidation(Repository repo,
    ...
    Set<String> deletedFiles = CommitUtils.getDeletedPaths(repo, c);
    checkForDuplicatesWithinThisSet(files, messages, locale);
    if (messages.isEmpty()) {
      try (TreeWalk tw = new TreeWalk(repo)) {
        CaseInsensitiveFilter f = new CaseInsensitiveFilter(files, locale);
        tw.setFilter(f);
        ...
        if (c.getParentCount() > 0) {
          ...
          while (tw.next()) {
            handleTreeWalkEntry(tw, files, deletedFiles, messages, locale);
          }
        }
      }
    }
  ...
  private static void handleTreeWalkEntry(TreeWalk tw, Set<String> files,
      Set<String> deletedFiles, List<CommitValidationMessage> messages,
    ...
    if (tw.isSubtree()) {
      ...
      for (String file : files) {
        if (file.length() == path.length()
            && !file.equals(path)
            && file.toLowerCase(locale).equals(path.toLowerCase(locale))) {
          messages.add(new CommitValidationMessage(
              file + ": pathname collides with " + path, true));
        }
      }
    }
  }
  ...
  private static void checkForDuplicatesWithinThisSet(Set<String> files,
    ...
    for (String file : files) {
      if (paths.containsKey(file.toLowerCase(locale))) {
        messages
            .add(new CommitValidationMessage(file + ": pathname collides with "
                + paths.get(file.toLowerCase(locale)), true));
      } else {
        paths.put(file.toLowerCase(locale), file);
      }
    }
  }
