...
public class DuplicatePathnameValidator implements CommitValidationListener {
  ...
  static List<CommitValidationMessage> performValidation(Repository repo,
    ...
    for (String p : pathnames) {
      changed.put(p.toLowerCase(locale), p);
    }

    Map<String, String> folders = DuplicateFilter.getAllParentFolders(pathnames, locale);

    try (TreeWalk tw = new TreeWalk(repo)) {
      DuplicateFilter f = new DuplicateFilter(pathnames, folders.values(), locale);
      tw.setFilter(f);
      tw.setRecursive(f.shouldBeRecursive());
      tw.addTree(c.getTree());
      while (tw.next()) {
        checkForDuplicates(tw, changed, messages, locale, folders);
      }
    }
    return messages;
  }

  private static void checkForDuplicates(TreeWalk tw,
      Map<String, String> changed, List<CommitValidationMessage> messages,
      Locale locale, Map<String, String> folders) {
    String path = tw.getPathString();
    String duplicate = changed.get(path.toLowerCase(locale));
    if(duplicate == null) {
      duplicate = folders.get(path.toLowerCase(locale));
    }
    if(duplicate != null) {
  ...
  private static void checkForDuplicatesWithinThisSet(Set<String> files,
    ...
    for(String file : files) {
      for(String parent : getAllParentFolders(file)) {
        if(!filesAndFolders.contains(parent)) {
          filesAndFolders.add(parent);
        }
      }
    }
