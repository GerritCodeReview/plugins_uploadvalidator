...
public class InvalidLineEndingValidator implements CommitValidationListener {
      ...
      try (Repository repo =
        ...
        List<CommitValidationMessage> messages =
            performValidation(repo, receiveEvent.commit,
                Arrays.asList(cfg.getStringList(KEY_IGNORE_FILES)));
        if (!messages.isEmpty()) {
  ...
  List<CommitValidationMessage> performValidation(Repository repo,
      RevCommit c, List<String> ignoreFiles) throws IOException {
    List<CommitValidationMessage> messages = new LinkedList<>();
    Map<String, ObjectId> content = CommitUtils.getChangedContent(repo, c);
    for (String path : content.keySet()) {
      if (ignoreFiles != null
          && ignoreFiles.contains(FilenameUtils.getExtension(path))) {
        continue;
      }
