...
public class MimeTypeValidator implements CommitValidationListener {
  ...
  @Inject
  MimeTypeValidator(@PluginName String pluginName, MimeUtil2 mimeUtil,
  ...
  public List<CommitValidationMessage> onCommitReceived(
    ...
    try {
      ...
      StringBuilder blockedTypes = new StringBuilder();
      for (String t : cfg.getStringList(KEY_BLOCKED_MIME_TYPE)) {
        blockedTypes.append('|').append(t);
      }
      Pattern blockedTypesPattern =
          Pattern.compile("^(" + blockedTypes.substring(1) + ")$");
      try (Repository repo =
          repoManager.openRepository(receiveEvent.project.getNameKey())) {
        List<CommitValidationMessage> messages =
            performValidation(repo, receiveEvent.commit, blockedTypesPattern,
                cfg.getBoolean(KEY_BLOCKED_MIME_TYPE_WHITELIST, false));
        if (!messages.isEmpty()) {
  ...
  List<CommitValidationMessage> performValidation(Repository repo, RevCommit c,
      Pattern blockedTypesPattern, boolean whitelist) throws IOException {
    ...
    for (String path : content.keySet()) {
      ...
      try (ObjectStream os = ol.openStream()) {
        String type = MimeUtil2
            .getMostSpecificMimeType(mimeUtil.getMimeTypes(os)).toString();
        if ((blockedTypesPattern.matcher(type).matches() && !whitelist)
            || !blockedTypesPattern.matcher(type).matches() && whitelist) {
          messages.add(new CommitValidationMessage(
              "found blocked mime type (" + type + ") in file: " + path, true));
        }
      }
    }
    return messages;
