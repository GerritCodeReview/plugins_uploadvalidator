...
public class MimeTypeValidator implements CommitValidationListener, Validator {
  ...
  MimeTypeValidator(@PluginName String pluginName,
      MimeUtil2 mimeUtil, PluginConfigFactory cfgFactory,
  ...
  public List<CommitValidationMessage> performValidation(Repository repo,
    ...
    Set<String> blockedTypes = new HashSet<>();
    for (String t : cfg.getStringList(KEY_BLOCKED_MIME_TYPE)) {
      blockedTypes.add(t.toLowerCase());
    }
    List<CommitValidationMessage> messages = new LinkedList<>();
    Map<String, ObjectId> content = ChangeUtils.getChangedContent(repo, c);
    for (String path : content.keySet()) {
      ...
      try (ObjectStream os = ol.openStream()) {
        ...
        if (blockedTypes.contains(type.toLowerCase())) {
