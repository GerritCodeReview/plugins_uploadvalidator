...
public class MimeTypeValidator implements CommitValidationListener {
  ...
  private static boolean isWhitelist(PluginConfig cfg) {
  ...
  public List<CommitValidationMessage> onCommitReceived(
    ...
    try {
      ...
      if (getBlockedTypes(cfg).length <= 0) {
      ...
      try (Repository repo =
        ...
        List<CommitValidationMessage> messages = performValidation(repo,
            receiveEvent.commit, getBlockedTypes(cfg), isWhitelist(cfg));
        if (!messages.isEmpty()) {
  ...
  List<CommitValidationMessage> performValidation(Repository repo, RevCommit c,
      String[] blockedTypes, boolean whitelist)
    ...
    for (String path : content.keySet()) {
      ...
      try (ObjectStream os = ol.openStream()) {
        ...
        if ((doesTypeMatch(type, blockedTypes) && !whitelist)
            || !doesTypeMatch(type, blockedTypes) && whitelist) {
  ...
  private boolean doesTypeMatch(String currentType, String[] blockedTypes)
