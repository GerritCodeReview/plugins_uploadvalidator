...
public class MimeTypeValidatorTest extends ValidatorTestCase {
  ...
  private RevCommit makeCommit()
    ...
    files.put(TestUtils.createEmptyFile("foo.html", repo),
        content.getBytes(StandardCharsets.UTF_8));
    files.put(TestUtils.createEmptyFile("foo.pdf", repo), getExamplePDF());
  ...
  public void testBlockedMimeType() throws Exception {
    ...
    .containsExactly(
        "ERROR: found blocked mime type (text/html) in file: foo.html",
        "ERROR: found blocked mime type (application/xml) in file: foo.xml",
        "ERROR: found blocked mime type (application/pdf) in file: foo.pdf");
  }
  ...
  public void validatorBehaviorWhenConfigEmpty() {
    assertThat(MimeTypeValidator.isActive(EMPTY_PLUGIN_CONFIG)).isFalse();
    assertThat(MimeTypeValidator.isWhitelist(EMPTY_PLUGIN_CONFIG)).isFalse();
  }
}
