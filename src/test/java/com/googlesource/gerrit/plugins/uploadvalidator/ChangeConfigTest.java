...
public class ChangeConfigTest {
  ...
  public void hasLegalAuthorEmailPattern() throws Exception {
    ...
    cfg.setString("plugins", pluginName, configKey, legalRegex);

    assertThat((configValidator.validateRegex(projectName, cfg, configKey)).isEmpty());
  }
  ...
  public void hasIllegalAuthorEmailPattern() throws Exception {
    String configKey = ChangeEmailValidator.KEY_ALLOWED_AUTHOR_EMAIL_PATTERN;
    cfg.setString("plugins", pluginName, configKey, illegalRegex);
    assertThat(!(configValidator.validateRegex(projectName, cfg, configKey)).isEmpty());
  }
  ...
  public void hasLegalMaxPathLength() throws Exception {
    ...
    cfg.setString("plugins", pluginName, configKey, legalPathLength);
    assertThat((configValidator.validateInteger(projectName, cfg, configKey)).isEmpty());
  }

  @Test
  public void hasIllegalMaxPathLength() throws Exception {
    String configKey = MaxPathLengthValidator.KEY_MAX_PATH_LENGTH;
    cfg.setString("plugins", pluginName, configKey, illegalPathLength);
